...
describe('DependencyGraph', () => {
  ...
  it('returns cyclic cells separately in getRecalcOrder', () => {
    const graph = new DependencyGraph();
    graph.setDependencies({ sheet: 'S1', col: 0, row: 0 }, [{ sheet: 'S1', col: 1, row: 0 }]);
    graph.setDependencies({ sheet: 'S1', col: 1, row: 0 }, [{ sheet: 'S1', col: 0, row: 0 }]);
    ...
    const { cyclic } = graph.getRecalcOrder(changedCells);
    const cyclicKeys = cyclic.map((a) => `${a.col},${a.row}`);
    expect(cyclicKeys).toContain('0,0');
    expect(cyclicKeys).toContain('1,0');
  });
});
